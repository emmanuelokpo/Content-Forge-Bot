...
# NOTE: Using MAKE_WEBHOOK_URL instead of SocialBu API Key
...
# This dict now stores platform copies AND product details (title, url) for distribution.
user_draft_content = {}
...
def generate_sales_copy(product_details):
    ...
    try:
        ...
        # Also include the product details in the final dictionary for the webhook
        content_dict.update({
...
def distribute_content_make_webhook(content_dict):
    """Sends content to a Make.com Webhook for distribution."""
    if not MAKE_WEBHOOK_URL:
        return "Make Webhook URL is missing. Distribution failed."

    # The payload is the data structure sent to Make.com
    payload = {
        "title": content_dict.get('title', 'Generated Content'),
        "url": content_dict.get('url', ''),
        "facebook_copy": content_dict.get('Facebook', ''),
        "x_copy": content_dict.get('X (Twitter)', ''),
        "linkedin_copy": content_dict.get('LinkedIn', ''),
        "pinterest_copy": content_dict.get('Pinterest', ''),
    }
    ...
    try:
        ...
        # Make.com returns a simple "Accepted" response (200) on success
        if response.status_code == 200:
             return f"Content successfully sent to Make.com Webhook (Status: 200). Your Make scenario should now be running!"
        else:
             return f"Make Webhook reported an issue. Status Code: {response.status_code}"

    except requests.exceptions.RequestException as e:
        logger.error(f"Make Webhook distribution error: {e}")
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "/approve - Use this command to instantly distribute the last reviewed content via Make.com.",
        parse_mode="Markdown"
    )
...
async def url_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # generated_dict now includes the title and url keys
    ...
    response_text = (
        ...
        f"**Product:** {generated_dict['title']}\n"
        f"**Source:** {generated_dict['url']}\n\n"
        f"--- **Review Content** ---\n\n"
...
async def approve_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handles the /approve command to distribute the content via Make.com."""
    user_id = update.effective_user.id
    ...
    await update.message.reply_text("🚀 Approval received! Initiating distribution to Make.com Webhook...")

    # 4. Distribution (Make.com)
    distribution_result = distribute_content_make_webhook(content_to_distribute)
